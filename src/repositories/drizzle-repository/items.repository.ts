...
export class ItemsRepository implements IItemsRepository {
  ...
  async create(itemData: CreateItemData) {
    const [item] = await this.databaseService.db.insert(itemsTable).values(itemData).returning();
    return item;
  }
  ...
  async findById(id: string, userId: string) {
    const whereCondition = and(
      eq(itemsTable.id, id),
      or(eq(itemsTable.userId, userId), eq(itemsTable.isPublic, false)),
    );

    const [item] = await this.databaseService.db.select().from(itemsTable).where(whereCondition);
    return item || null;
  }

  async update(id: string, itemData: UpdateItemData) {
    const [item] = await this.databaseService.db.transaction(
      async () =>
  ...
  async delete(id: string) {
    const [item] = await this.databaseService.db.transaction(
      async () =>
        await this.databaseService.db.delete(itemsTable).where(eq(itemsTable.id, id)).returning(),
    );
